...
class FCN(nn.Module):
    ...
    def relative_error_l2_norm(self, x_BC, y_BC):
        error_u_predict = torch.norm(self.forward(x_BC) - y_BC)
        norm_u_real = torch.norm(y_BC)
        return error_u_predict / norm_u_real
